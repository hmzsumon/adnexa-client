...
const accentClass = {
  ...
  violet:
    "border-violet-400/25 from-violet-500/18 to-fuchsia-500/5 text-violet-300",
...
const MethodCard = ({
  ...
  const content = (
    <div
      className={`adnexa-glass-card group rounded-2xl my-4 border bg-gradient-to-br p-4 transition-all duration-300 ${accentClass[accent]} ${isActive ? "hover:-translate-y-1" : "opacity-70"}`}
    >
      {/* ────────── Method Header ────────── */}
      <div className="flex items-center justify-between gap-3">
        <div className="flex min-w-0 items-center gap-3">
          <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-xl bg-white/10 text-3xl">
            {image ? (
              <img
                src={image}
                alt={title}
          ...
          <div className="min-w-0">
            ...
            <span
              className={`mt-1 inline-flex items-center gap-1 rounded-full px-3 py-1 text-[11px] font-black ${isActive ? "bg-emerald-400/12 text-emerald-300" : "bg-slate-500/12 text-slate-400"}`}
            >
        ...
        <HiChevronRight
          className={`shrink-0 text-2xl ${isActive ? "text-white/80 group-hover:translate-x-1" : "text-slate-500"} transition-transform`}
        />
      </div>
